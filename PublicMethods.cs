...
namespace PVPNetConnect
{
    public partial class PVPNetConnection
    {
        ...
        public void SelectDefaultMasteryBookPage(MasteryBookPageDTO masteryBookPage,
            MasteryBookPageDTO.Callback callback)
        {
            MasteryBookPageDTO cb = new MasteryBookPageDTO(callback);
            InvokeWithCallback("masteryBookService", "selectDefaultMasteryBookPage",
                new object[] {masteryBookPage.GetBaseTypedObject()}, cb);
        }

        public async Task<MasteryBookPageDTO> SelectDefaultMasteryBookPage(MasteryBookPageDTO masteryBookPage)
        {
            int Id = Invoke("masteryBookService", "selectDefaultMasteryBookPage",
                new object[] {masteryBookPage.GetBaseTypedObject()});
            while (!results.ContainsKey(Id))
                await Task.Delay(10);
            TypedObject messageBody = results[Id].GetTO("data").GetTO("body");
            MasteryBookPageDTO result = new MasteryBookPageDTO(messageBody);
            results.Remove(Id);
            return result;
        }
        ...
        public async Task<object> SaveMasteryBook(MasteryBookDTO Spellbook)
        {
            int Id = Invoke("masteryBookService", "saveMasteryBook", new object[] {Spellbook});
            while (!results.ContainsKey(Id))
                await Task.Delay(10);
            results.Remove(Id);
            return null;
